...
class TalkingHead {
  ...
  constructor(node, opt = null) {
    ...
    this.onStartSpeaking = opt.onStartSpeaking || function () {};
    this.onStopSpeaking = opt.onStopSpeaking || function () {};
    ...
    this.animMoods = {
      neutral: {
        ...
        anims: [
          ...
          },
          {
            ...
            vs: { eyesRotateY: [[-0.6, 0.6]], eyesRotateX: [[-0.2, 0.6]] },
          },
          {
            name: "blink",
            delay: [1000, 8000, 1, 2],
          ...
          },
          {
            ...
            vs: {
              eyeSquintLeft: [[0, 0.3, 3]],
              eyeSquintRight: [[0, 0.3, 3]],
              browInnerUp: [[0, 0.3]],
              browOuterUpLeft: [[0, 0.3]],
              browOuterUpRight: [[0, 0.3]],
            },
          },
        ],
      },
      ...
      fear: {
        ...
        anims: [
          ...
          },
          {
            ...
            vs: { eyesRotateY: [[-1, 1]], eyesRotateX: [[-0.2, 0.6]] },
          },
          {
            name: "blink",
            delay: [2000, 8000, 1, 2],
      ...
      disgust: {
        ...
        anims: [
          ...
          {
            name: "pose",
            alt: [{ delay: [5000, 10000], vs: { pose: ["side"] } }],
          },
          {
            name: "head",
            idle: {
      ...
      love: {
        ...
        anims: [
          ...
          },
          {
            ...
            delay: [2000, 8000, 1, 2],
            dt: [50, [200, 300], 100],
            vs: { eyeBlinkLeft: [0.6, 0.6, 0], eyeBlinkRight: [0.6, 0.6, 0] },
          },
          {
            name: "mouth",
            delay: [1000, 5000],
          ...
          },
          {
            ...
            vs: {
              ...
              eyeSquintRight: [[0, 0.3, 3]],
              browInnerUp: [[0.3, 0.6]],
              browOuterUpLeft: [[0.1, 0.3]],
              browOuterUpRight: [[0.1, 0.3]],
            },
          },
        ],
      },
    ...
    this.animEmojis = {
      ...
      "😉": {
        ...
        vs: {
          ...
          eyeBlinkRight: [0, 0, 0],
          headRotateX: [0.05, 0.05, 0.05, 0],
          headRotateZ: [-0.05, -0.05, -0.05, 0],
      ...
      "😘": {
        ...
        vs: {
          ...
          eyesRotateY: [0],
          headRotateY: [0],
          headRotateX: [0, 0.05, 0.05, 0],
          headRotateZ: [0, -0.05, -0.05, 0],
          eyeSquintLeft: [1],
          eyeSquintRight: [1],
          mouthPucker: [0, 0.5, 0],
          noseSneerLeft: [0, 0.7],
  ...
  setMood(s) {
    ...
    for (let mt of [
      ...
      this.setBaselineValue(
        mt,
        this.mood.baseline.hasOwnProperty(mt) ? this.mood.baseline[mt] : 0
      );
    }
  ...
  getBaselineValue(mt) {
    if (mt === "eyesRotateY") {
      ...
      return this.animBaseline.hasOwnProperty(mt)
        ? this.animBaseline[mt].target
        : undefined;
    }
  }
  ...
  getFixedValue(mt) {
    if (mt === "eyesRotateY") {
      ...
      return this.animFixed.hasOwnProperty(mt)
        ? this.animFixed[mt].target
        : undefined;
    }
  }
  ...
  animFactory(t, loop = false, scaleTime = 1, scaleValue = 1) {
    ...
    const delay = a.delay
      ? Array.isArray(a.delay)
        ? this.gaussianRandom(...a.delay)
        : a.delay
      : 0;
    if (a.hasOwnProperty("dt")) {
      a.dt.forEach((x, i) => {
        o.ts[i + 1] =
          o.ts[i] + (Array.isArray(x) ? this.gaussianRandom(...x) : x);
      });
    }
    ...
    for (let mt of Object.keys(o.vs)) {
      while (o.vs[mt].length - 1 < o.ts.length)
  ...
  valueAnimationSeq(ts, vs, t, fun = null) {
    ...
    if (t <= ts[iMin])
      return typeof vs[iMin] === "function" ? vs[iMin]() : vs[iMin];
    if (t >= ts[iMax])
      return typeof vs[iMax] === "function" ? vs[iMax]() : vs[iMax];
    while (t > ts[iMin + 1]) iMin++;
    iMax = iMin + 1;
    let k =
      ((typeof vs[iMax] === "function" ? vs[iMax]() : vs[iMax]) -
        (typeof vs[iMin] === "function" ? vs[iMin]() : vs[iMin])) /
      (ts[iMax] - ts[iMin]);
    if (fun) k = fun((t - ts[iMin]) / (ts[iMax] - ts[iMin])) * k;
    const b =
      (typeof vs[iMin] === "function" ? vs[iMin]() : vs[iMin]) - k * ts[iMin];
    return k * t + b;
  }
  ...
  animate(t) {
    ...
    if (this.viewName !== "full") {
      ...
      if (v === vb) {
        const randomizedV =
          (this.mood.baseline[randomizedMs] || 0) + Math.random() / 5;
      ...
      if (this.animClock >= x.ts[0]) {
        for (let [mt, vs] of Object.entries(x.vs)) {
          if (mt === "subtitles") {
            ...
            if (this.animBaseline.hasOwnProperty(mt))
              this.animBaseline[mt].t0 = undefined;
            for (let j = 0; j < i; j++) {
              if (this.animQueue[j].vs.hasOwnProperty(mt))
                delete this.animQueue[j].vs[mt];
            }
          }
        }
    ...
    for (let [mt, x] of Object.entries(o)) {
      if (mt === "subtitles") {
        ...
        Object.entries(x.props).forEach((e) => {
          ...
          this.poseTarget.props[e[0]].d =
            e[1] && e[1].d ? e[1].d : x.duration || 2000;
        });
      } else if (mt === "handLeft") {
        this.ikSolve(
          {
          ...
          x.x ? new THREE.Vector3(x.x, x.y, x.z) : null,
          true,
          x.d
        );
  ...
  speakText(s, opt = null, onsubtitles = null, excludes = null) {
    ...
    const speakables = /[\p{L}\p{N},\.'!€\$\+\p{Dash_Punctuation}%&\?]/gu;
      ...
      for (let i = 0; i < r.words.length; i++) {
        ...
        if (word.length) {
          ...
          if (!r.visemes) {
            ...
            if (v && v.visemes && v.visemes.length) {
              const dTotal =
                v.times[v.visemes.length - 1] +
                v.durations[v.visemes.length - 1];
              const overdrive = Math.min(
                duration,
                Math.max(0, duration - v.visemes.length * 150)
              );
              let level =
  ...
  async startSpeaking(force = false) {
    ...
    this.onStartSpeaking(); // Trigger the start speaking event
  ...
  stopSpeaking() {
    ...
    this.stateName = "idle";
    this.isSpeaking = false;
    this.onStopSpeaking(); // Trigger the stop speaking event
